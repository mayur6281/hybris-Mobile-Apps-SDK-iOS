...
- (void)postHealthDataForURL:(NSString*)url inputData:(NSData*)data withCompletionBlock:(NSDictionaryNSErrorBlock)completionBlock
{
...
//For Sending Geolocation Data

- (NSString *)urlForgreetCustomer {
    NSString *url = [NSString stringWithFormat:@"%@%@greetCustomer/",
                     self.baseURL,
                     self.siteURLSuffix];
...
- (NSString*)getProfileImageURL
{
    ...
    NSString *contentURL = @"/bncwebservices/v1/electronics/customerImages";
...
- (void)GetProfilePictureURL:(NSString *)CustomerID completionBlock:(NSDictionaryNSErrorBlock)completionBlock
{
    ...
    [self clientCredentialsTokenWithCompletionBlock:^(NSDictionary *dict, NSError *error) {
        ...
        NSDictionary *inputData = [[NSDictionary alloc]initWithObjectsAndKeys:CustomerID, @"customerId",nil];

        
        NSData *jsonInputData = [NSJSONSerialization dataWithJSONObject:inputData options:NSJSONWritingPrettyPrinted error:&error];
        
        NSString *postBody = [[NSString alloc] initWithData:jsonInputData encoding:NSUTF8StringEncoding];
        postBody=[postBody stringByReplacingOccurrencesOfString:@"\n" withString:@""];
        postBody=[postBody stringByReplacingOccurrencesOfString:@" " withString:@""];
        postBody=[postBody stringByReplacingOccurrencesOfString:@"\\" withString:@""];
        
        
        NSData *postData = [NSData dataWithBytes:[postBody UTF8String] length:[postBody length]];
        
        [[HYWebServiceDataProvider alloc] authorizedURLForJSON:[self getProfileImageURL] httpMethod:@"POST" httpBody:postData completionBlock:^(NSData *jsonData, NSError *error)
         
         //        [[HYWebServiceDataProvider alloc] authorizedURL:[self urlForgreetCustomer] clientCredentialsToken:tempAuthToken httpBody:postData completionBlock:^(
         //                                                                                                                                                            NSData *jsonData, NSError *error)
         {
             if (jsonData) {
                 if (completionBlock) {
                     ...
                     //                    {
                     //                        description = [promotion objectForKey:@"description"];
                     ////                        UIAlertView *alert =
                     ////                        [[UIAlertView alloc] initWithTitle:NSLocalizedString(strWelcome, @"Success alert box title") message:NSLocalizedString(description, @"") delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", @"OK button") otherButtonTitles:nil];
                     ////                        [alert show];
                     //
                     //                    }
                     //
                     ...
                     if (jsonError) {
                         if (completionBlock) {
                             dispatch_async (dispatch_get_main_queue (), ^{ completionBlock (dict,jsonError);
                             });
                         }
                     ...
                     if (completionBlock) {
                         dispatch_async (dispatch_get_main_queue (), ^{ completionBlock (dict,[NSError errorWithDomain:@"com.hybris" code:1 userInfo:dict]);
                         });
                     }
                 }
                 else {
...
//For Sending Geolocation data of the user

- (void)GreetCustomerWithLat:(NSString *)Lattitude
                       Longi:(NSString *)Longitude
                 CustEmailId:(NSString *)CustomerEmailID
             completionBlock:(NSDictionaryNSErrorBlock)completionBlock {
    //Generate credentials auth_token
    [self clientCredentialsTokenWithCompletionBlock:^(NSDictionary *dict, NSError *error) {
        if (completionBlock) {
        ...
        [[HYWebServiceDataProvider alloc] authorizedURLForJSON:[self urlForgreetCustomer] httpMethod:@"POST" httpBody:postData completionBlock:^(NSData *jsonData, NSError *error)
